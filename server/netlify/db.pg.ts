...
export async function getDb() {
  const connectionString = process.env.INVOICEPRO_DATABASE_URL ?? process.env.DATABASE_URL;
...
export async function createOwnerAccount(input: { name: string; email: string; passwordHash: string }) {
  const db = await getDb();
  const email = input.email.toLowerCase();
  const existing = await getUserByEmail(email);
  if (existing) return existing;
  const [owner] = await db.insert(users).values({
    openId: `owner_${randomBytes(18).toString("base64url")}`,
    name: input.name.trim(),
    email,
    loginMethod: "password",
    role: "admin",
  ...
  await db.insert(customerCredentials).values({ userId: owner.id, passwordHash: input.passwordHash });
  await logActivity(owner.id, "account", null, "owner_created", "Created the initial InvoicePro owner account");
  return owner;
}
...
export async function saveProfile(
  ...
  const db = await getDb();
  await db.insert(freelancerProfiles).values({ userId, ...input }).onConflictDoUpdate({ target: freelancerProfiles.userId, set: { ...input, updatedAt: new Date() } });
...
export async function createClient(
  ...
  const db = await getDb();
  const [result] = await db.insert(clients).values({ userId, ...input }).returning({ id: clients.id });
  const id = result.id;
...
export async function updateClient(
  ...
  const db = await getDb();
  const result = await db.update(clients).set({ ...input, updatedAt: new Date() }).where(and(eq(clients.id, clientId), eq(clients.userId, userId))).returning({ id: clients.id });
...
export async function deleteClient(userId: number, clientId: number) {
  const db = await getDb();
  const result = await db.delete(clients).where(and(eq(clients.id, clientId), eq(clients.userId, userId))).returning({ id: clients.id });
  if (result.length === 0) throw new Error("Client not found");
...
export async function updateInvoiceStatus(userId: number, invoiceId: number, status: (typeof invoiceStatusValues)[number]) {
  const db = await getDb();
  const invoice = await getInvoiceDetail(userId, invoiceId);
...
export async function savePlatformSettings(adminUserId: number, input: {
  ...
  await db.insert(platformSettings).values(values).onConflictDoUpdate({
